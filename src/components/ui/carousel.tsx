...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
    ...
    return (
        <li
            ref={slideRef}
            className="flex flex-1 flex-col items-center justify-center relative z-10 w-[300px] sm:w-[450px] h-[400px] sm:h-[550px] opacity-100 transition-all duration-600 ease-in-out"
        ...
        >
            ...
            >
                ...
                <div className="flex justify-center mt-4">
                    <button className="px-4 py-2 mt-4 text-sm text-black bg-white border border-transparent rounded-full shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] font-bold">
                        {button}
                    </button>
                </div>
            </article>
...
export default function Carousel({ slides }: CarouselProps) {
    ...
    return (
        <div className="relative w-full h-[70vmin]" aria-labelledby={`carousel-heading-${id}`}>
            <ul
                className="absolute flex gap-[4vmin] transition-transform duration-600 ease-in-out"
                style={{
                    transform: `translateX(-${current * (70 + 4)}vmin)`,
                }}
            >
